"""
UBS Kontoauszug PDF parser.

Extracts transactions from UBS bank statement PDFs using
word-level position parsing with pdfplumber.
"""
...
def extract_transactions_from_pdf(pdf_file) -> list[dict]:
    """
    ...
    with pdfplumber.open(pdf_file) as pdf:
        for page in pdf.pages:
            ...
            valuta_left = headers.get("Valuta", {}).get("x0", 430)
            ...
            for y in sorted(merged.keys()):
                ...
                if date_pattern.match(datum_text.strip()):
                    ...
                    if any(kw in info_lower for kw in [
                        "anfangssaldo", "schlusssaldo", "umsatztotal", "formular ohne"
                    ]):
                        continue
    ...
    # Clean boilerplate
    boilerplate = [
        r"\s*Sie,.*benachrichtigen.*",
        r"\s*Grüße.*$",
        r"\s*hne Unterschrift.*",
    ]
