"""
Email sender for bookkeeping data — sends Banana TXT and CSV as attachments.
"""
...
from core.export import df_to_banana_tsv, df_to_csv
...
def get_smtp_config() -> dict:
    """Public accessor for SMTP config."""
    return _load_smtp_config()
...
def send_bookkeeping_email(
    ...
    """Send bookkeeping data as Banana TXT + CSV attachments.
    ...
    if not cfg["host"] or not cfg["user"]:
        return False, "E-Mail nicht konfiguriert. Bitte .env Datei prüfen."
    ...
    if not body_text:
        ...
        try:
            total = df["Betrag CHF"].apply(
                lambda x: float(x) if x != "" and x is not None and not (isinstance(x, float) and pd.isna(x)) else 0
            ).sum()
        except Exception:
            pass
        body_text = (
            f"Buchhaltungsdaten vom {today}\n\n"
            f"Anzahl Buchungen: {n_rows}\n"
    ...
    try:
        ...
        return False, "SMTP Anmeldung fehlgeschlagen. Bitte Zugangsdaten in .env prüfen."
