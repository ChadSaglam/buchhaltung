      ...
      {f.isLoading ? (
        ...
        <>
          ...
          <section aria-labelledby="adresse-titel" className="rounded-xl border border-border bg-card p-6">
            ...
            <SettingsField label="Firmenname" description="Steht als Zahlungsempfänger im Zahlteil">
              <SettingsInput label="Firmenname" value={f.entwurf.name} onChange={(v) => f.setFeld("name", v)} />
            </SettingsField>
            <SettingsField label="Strasse und Nr.">
              <div className="flex gap-2">
                <SettingsInput label="Strasse" value={f.entwurf.strasse} onChange={(v) => f.setFeld("strasse", v)} />
                <div className="w-24">
                  <SettingsInput label="Hausnummer" value={f.entwurf.hausnummer} onChange={(v) => f.setFeld("hausnummer", v)} />
                </div>
              </div>
            </SettingsField>
            <SettingsField label="PLZ und Ort">
              <div className="flex gap-2">
                <div className="w-28">
                  <SettingsInput label="PLZ" value={f.entwurf.plz} onChange={(v) => f.setFeld("plz", v)} />
                </div>
                <SettingsInput label="Ort" value={f.entwurf.ort} onChange={(v) => f.setFeld("ort", v)} />
              </div>
            </SettingsField>
            ...
            <SettingsField label="MWST-Nummer" description="z. B. CHE-123.456.789 MWST">
              <SettingsInput label="MWST-Nummer" value={f.entwurf.mwst_nr} onChange={(v) => f.setFeld("mwst_nr", v)} />
            </SettingsField>
          </section>
          ...
          <section aria-labelledby="zahlung-titel" className="rounded-xl border border-border bg-card p-6">
            ...
            <SettingsField label="IBAN" description={f.profil?.iban_formatiert || "CH.. oder LI.., QR-IBAN wird erkannt"}>
              <SettingsInput label="IBAN" value={f.entwurf.iban} onChange={(v) => f.setFeld("iban", v)} placeholder="CH44 3199 9123 0008 8901 2" />
            </SettingsField>
